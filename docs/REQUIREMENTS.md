...
| 歌詞読み仮名 | kuromoji + 分析結果を歌詞横に表示 |
| UI | ダークテーマ、**iPhone 下部タブ**、日本語 |
| メモ帳 | `--hook` / `--verse`、反映→自動分析 |
| 歌詞手動編集 | 編集 + 分析更新 |
| プロジェクト保存 | ローカル（歌詞+韻+Setup+分析+メモ） |
| クラウド保存 | Supabase 歌詞履歴（本番接続済）→ [SUPABASE_SETUP.md](./SUPABASE_SETUP.md) |
| 韻候補タップ挿入 | メモ帳 / 歌詞 |
| 韻だけ取得 | Setup ボタン |
| 完了トースト | 生成・韻・分析・保存 |
| TXT エクスポート | 歌詞・韻候補 |
...
| 機能 | 優先度 |
|------|--------|
| Google OAuth + プロジェクト全体のクラウド同期 | 高 |
| 歌詞編集後の自動デバウンス分析 | 中 |
| メモ帳タグのプレビュー（サビ/バース色分け） | 中 |
| MeCab（VPS） | 低 |
...
| POST /api/save-lyrics | Supabase に歌詞履歴保存 |
