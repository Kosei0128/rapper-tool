...
/** ラップで響く末尾パターン */
function hasRapEnding(word: string): boolean {
  return /(?:者|[ジャじゃャ])$/.test(word);
}
...
export function scoreRhymeCandidate(
  ...
  if (source === "azrhymes") {
    ...
    if (hasRapEnding(cWord)) score += 0.02;
  ...
  if (source === "in-note") {
    ...
    if (hasRapEnding(cWord)) score += 0.1;
  ...
  if (queryVowels && candidate.vowels) {
    ...
    if (tail >= 0.5 && !hasRapEnding(cWord) && cWord.length > 4) {
      score -= 0.15;
    }
  }
